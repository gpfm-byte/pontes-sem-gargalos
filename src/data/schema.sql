...
SELECT create_hypertable('leituras_sensor', 'timestamp');
...
SELECT add_compression_policy('leituras_sensor', INTERVAL '7 days');
...
CREATE INDEX idx_leituras_ponte_ts ON leituras_sensor (ponte_id, timestamp DESC);
CREATE INDEX idx_alertas_ativos    ON alertas (ponte_id, criado_em DESC) WHERE resolvido_em IS NULL;
CREATE INDEX idx_previsoes_ponte   ON previsoes_ia (ponte_id, gerada_em DESC);

-- View útil: status atual de cada ponte (última leitura)
CREATE VIEW status_atual_pontes AS
